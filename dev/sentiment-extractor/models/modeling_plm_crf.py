...
from transformers.modeling_electra import ElectraPreTrainedModel, ElectraModel, ElectraConfig
...
class BertCFRForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.crf = CRF(num_tags=config.num_labels,ignore_index=0, default_label_pad_index=None) # ignore_index and label padding == -100
        self.init_weights()
    ...
    def forward(
        ...
        # token_classification layer
        logits = self.classifier(sequence_output)
    
        
        loss = None
        if labels is not None: # calculate crf loss
            loss = self.crf(emissions=logits, tags=labels)
            loss = -1 * loss # negative log-likelihood    
        
        output = (logits,) + outputs[2:] # logits, hidden_states, attentions
        return ((loss,) + output) if loss is not None else output # add loss
...
class ElectraCRFForTokenClassification(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.crf = CRF(num_tags=config.num_labels,ignore_index=0, default_label_pad_index=None) 
    ...
    def forward(
        ...
        discriminator_hidden_states = self.electra(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        # token embeddings
        ...
        output = (logits,) + discriminator_hidden_states[1:] # logits, hidden_states, attentions
        return ((loss,) + output) if loss is not None else output # add loss